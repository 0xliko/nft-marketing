...
const Header: React.FC<IProps> = ({landing}) => {
    ...
    return (
        <>
            ...
            <div className={`navbar-custom ${landing ? "landing" : ""}`}>
                <div className="header-container">
                    <div className="menu-container float-right pl-lg-2 pl-md-1">
                        <a className={`text-white-1 float-left sidebar-toggle ${landing?"landing":""}`}
                           onClick={() => {
                        ...
                        <a className={ClassNames("link", {active: location.pathname == "/about"})}
                           href="/about">About</a>
                ...
                <MDBModal size="sm" isOpen={modalOpened} toggle={() => {
                    setModalOpened(!modalOpened)
                }} centered={true}>
                    <MDBModalHeader toggle={() => {
                        setModalOpened(!modalOpened)
                    }}>Select Wallet Provider</MDBModalHeader>
                    <MDBModalBody>
                        {
                            connectors.map((connectorInfo: IConnector, key: number) => {
                                ...
                                return <MDBBtn key={key} rounded color="gray" tag="a" outline={true}
                                               className={`provider-button ${connected ? "connected" : ""}`}
                                               onClick={() => {
                                                   connectToWallet(connectorInfo)
                                               }}>
